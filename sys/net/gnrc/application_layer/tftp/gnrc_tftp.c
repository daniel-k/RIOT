...
int gnrc_tftp_client_read(ipv6_addr_t *addr, const char *file_name,
                          tftp_data_callback data_cb, tftp_transfer_start_callback start_cb, bool use_option_extensions) {
    tftp_context_t ctxt;
    ...
    if (use_option_extensions) {
        if (_tftp_set_opts(&ctxt, mtu, GNRC_TFTP_DEFAULT_TIMEOUT, 0) != FINISHED) {
            return -EINVAL;
        }
    } else
...
int gnrc_tftp_client_write(ipv6_addr_t *addr, const char *file_name,
                           tftp_data_callback data_cb, uint32_t total_size, bool use_option_extensions) {
    tftp_context_t ctxt;

    /* prepare the context */
    if (_tftp_init_ctxt(addr, file_name, NULL, data_cb, TO_WRQ, &ctxt) < 0) {
        return -EINVAL;
    }
    ...
    uint16_t mtu = _tftp_get_maximum_block_size();
    if (use_option_extensions) {
        if (_tftp_set_opts(&ctxt, mtu, GNRC_TFTP_DEFAULT_TIMEOUT, total_size) != FINISHED) {
            return -EINVAL;
        }
    } else
        _tftp_set_default_options(&ctxt);
...
tftp_state _tftp_state_processes(tftp_context_t *ctxt, msg_t *m) {
    ...
    case TO_WRQ: {
        ...
        if (!ctxt->start_cb(action, ctxt->file_name, ctxt->transfer_size)) {
